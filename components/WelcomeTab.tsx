      ...
      {showAdd && (
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[3rem] shadow-xl border border-indigo-50 space-y-6 animate-in slide-in-from-top-4 duration-300">
          <label className="w-full aspect-[4/3] bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center cursor-pointer overflow-hidden relative group hover:border-indigo-300 transition-colors">
            {formData.imageData ? <img src={formData.imageData} className="w-full h-full object-cover" /> : <div className="flex flex-col items-center space-y-2"><ImageIcon size={28} /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Select Visual</span></div>}
          ...
          <div key={t.id} className={`bg-white p-5 rounded-[2.5rem] border-2 transition-all flex items-center space-x-5 ${t.isActive ? 'border-emerald-400 bg-emerald-50 shadow-2xl' : 'border-white shadow-sm'}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex space-x-2">
                {t.isActive ? <button onClick={() => onStop()} className="flex-1 bg-rose-600 text-white px-4 py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center space-x-2"><Square size={12} fill="white" /><span>Stop</span></button> : <button onClick={() => onStart(t.id)} className="flex-1 bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center space-x-2"><Play size={12} fill="white" /><span>Start</span></button>}
                <button onClick={() => onDelete(t.id)} className="bg-white text-rose-500 p-3 rounded-xl hover:bg-rose-50 border border-slate-200 shadow-sm shrink-0"><Trash2 size={18} strokeWidth={2.5} /></button>
              </div>
            </div>
          </div>
        ))}
