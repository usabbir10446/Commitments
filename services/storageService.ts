...
export const storageService = {
  ...
  saveTask: async (task: Partial<Task>): Promise<Task | null> => {
    const payload = { date: task.date, time_block: task.timeBlock, title: task.title, venue: task.venue, remarks: task.remarks, attended: task.attended };
    let result;
    if (task.id) result = await supabase.from('tasks').update(payload).eq('id', task.id).select().single();
    else result = await supabase.from('tasks').insert([payload]).select().single();
    if (result.error) return null;
    const t = result.data;
    return { id: t.id, date: t.date, timeBlock: t.time_block, title: t.title, venue: t.venue, remarks: t.remarks, attended: t.attended };
  },
  ...
  getWelcomeTasks: async (): Promise<WelcomeTask[]> => {
    ...
    if (error) return [];
    return (data || []).map(d => ({ id: d.id, topText: d.top_text, bottomText1: d.bottom_text_1, bottomText2: d.bottom_text_2, bottomText3: d.bottom_text_3, imageData: d.image_data, isActive: d.is_active }));
  },
  saveWelcomeTask: async (wt: Partial<WelcomeTask>): Promise<boolean> => {
    const payload = { top_text: wt.topText, bottom_text_1: wt.bottomText1, bottom_text_2: wt.bottomText2, bottom_text_3: wt.bottomText3, image_data: wt.imageData };
  ...
  getEmergencies: async (): Promise<EmergencyMessage[]> => {
    ...
    if (error) return [];
    return (data || []).map(e => ({ id: e.id, text: e.text, createdAt: new Date(e.created_at).getTime() }));
  },
